...
const CLIPBOARD_HEADER =
  CONTAINER.querySelector(".plotlyplot-clipboard-header") ??
  CONTAINER.insertAdjacentElement(
    "afterbegin",
    html`<div class="plotlyplot-clipboard-header hidden">
...
// The span getters and setters below read and write this object, so make sure
// it exists on every run: a re-render replaces plot_obj, and the new config
// may come without image options.
...
for (const [key, value] of Object.entries(getImageOptions())) {
  ...
  if (firstRun) {
    ...
    // On a re-render the spans already exist: sync them to the image options
    // of the new plot object.
    container.ui_value = value;
  }
}
...
CLIPBOARD_HEADER.onmousedown = function (event) {
  ...
  document.addEventListener(
    "mousedown",
    (e) => {
      if (e.target.closest(".plotlyplot-container") !== CONTAINER) {
        cleanUp();
        controller.abort();
        return;
      }
...
// Show a short acknowledgement on the modebar clipboard button after a
// successful copy. Repeated clicks reset the timer instead of stacking.
let copy_ack_timer;
function acknowledgeCopy() {
  const button = CONTAINER.querySelector('[data-title="Copy PNG to Clipboard"]');
  if (!button) {
    return;
  }
  button.classList.add("plotlyplot-copied");
  clearTimeout(copy_ack_timer);
  copy_ack_timer = setTimeout(() => {
    button.classList.remove("plotlyplot-copied");
  }, 1000);
}
...
function copyImageToClipboard() {
  ...
  Plotly.toImage(PLOT, config)
    .then(function (dataUrl) {
      return fetch(dataUrl)
        .then((res) => res.blob())
        .then(function (blob) {
          const paste_receiver = document.querySelector('paste-receiver.plotlyplot')
          if (paste_receiver) {
            paste_receiver.attachImage(dataUrl, CONTAINER)
          }
          return sendToClipboard(blob);
        });
    })
